...
def run_network_NeRFW(inputs, viewdirs, ts, fn, embed_fn, embeddirs_fn,
    ...
    if typ == 'coarse' and test_time:
        inputs_flat = torch.reshape(inputs, [-1, inputs.shape[-1]]) # 19200=120(img_height)*160(img_width)

        # Feed NeRF coarse train
        for i in range(0, inputs_flat.shape[0], netchunk):
            embedded_inputs = [embed_fn(inputs_flat[i: i + netchunk])]
            out_chunks += [fn(torch.cat(embedded_inputs, 1), sigma_only=True)] # 65536=1024(rays_num)*64(sample_points)
...
def create_nerf(args):
    ...
    if args.use_viewdirs:
        if args.reduce_embedding == 2:  # use DNeRF embedding
            if args.no_DNeRF_viewdir:  # no DNeRF embedding for viewdir
                ...
                embeddirs_fn, input_ch_views, embedddirs_obj = get_embedder(args.multires_views, args.i_embed, args.reduce_embedding,
                                                                            args.epochToMaxFreq)
        else:
    ...
    if args.NeRFH:
        ...
        model = NeRF(D=args.netdepth, W=args.netwidth, input_ch=input_ch, output_ch=output_ch, skips=skips, input_ch_views=input_ch_views,
    ...
    if args.NeRFH:
        network_query_fn = lambda inputs, viewdirs, ts, network_fn, typ, embedding_a, embedding_t, output_transient, test_time: (
            run_network_NeRFW(inputs, viewdirs, ts, network_fn, embed_fn=embed_fn, embeddirs_fn=embeddirs_fn, typ=typ, embedding_a=embedding_a,
                              embedding_t=embedding_t, output_transient=output_transient, netchunk=args.netchunk, test_time=test_time))
    else:
