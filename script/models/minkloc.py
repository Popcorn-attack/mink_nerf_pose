...
class MinkLoc(torch.nn.Module):
    ...
    def forward(self, batch):
        #print features and coords device   
        # Coords must be on CPU, features can be on GPU - see MinkowskiEngine documentation
        print(f"features device: {batch['features'].device}")
        print(f"coords device: {batch['coords'].device}")
        x = ME.SparseTensor(features=batch['features'].to('cpu'), coordinates=batch['coords'].to('cpu'))
        x = self.backbone(x)
