...
# from utils.utils import psnr, ssim, lpips
...
def raw2outputs_NeRFW(raw, z_vals, rays_d, raw_noise_std=0.0, output_transient=False, beta_min=0.1, white_bkgd=False,
                      test_time=False,
                      static_only=True, typ="coarse"):
    ...
    if typ == "coarse" and test_time:
        ...
        depth_var = None
        beta = None
        return rgb_map, disp_map, acc_map, weights, depth_map, depth_var, transient_sigmas, beta
    ...
    if output_transient:
        ...
        if test_time and static_only:  # output static rgbs only, need testing
            ...
            # get depth_var for depth loss
            dm = depth_map.unsqueeze(1)
            depth_var = reduce(weights * (dm - z_vals) * (dm - z_vals), 'n1 n2 -> n1', 'sum')
            return rgb_map, disp_map, acc_map, weights, depth_map, depth_var, transient_sigmas, beta
...
def render_rays(ray_batch,
    ...
    # [N_rays, N_samples, 3] # Sample 3D point print it out, [batchsize, 64 samples, 3 axis]
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, depth_var, _, _ = raw2outputs_NeRFW(raw, z_vals, rays_d,
    ...
    if N_importance > 0:
        ...
        if i_epoch >= 0:  # Coarse-to-fine
            raw = network_query_fn(pts, viewdirs, img_idxs, network_fine, 'fine', embedding_a, embedding_t,
                                   output_transient,
                                   test_time=test_time, epoch=i_epoch)
        else:
            raw = network_query_fn(pts, viewdirs, img_idxs, network_fine, 'fine', embedding_a, embedding_t,
                                   output_transient,
                                   test_time=test_time)

        rgb_map, disp_map, acc_map, weights, depth_map, depth_var, transient_sigmas, beta = raw2outputs_NeRFW(raw,
                                                                                                              z_vals,
                                                                                                              rays_d,
                                                                                                              raw_noise_std,
                                                                                                              output_transient,
    ...
    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map, 'depth': depth_map, 'depth_var': depth_var}
    if retraw:
        ret['raw'] = raw
...
def render(H, W, focal, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           near=0.0, far=1.0, use_viewdirs=False, c2w_staticcam=None, img_idx=torch.Tensor(0),
           **kwargs):
    if c2w is not None:
    ...
    rays = torch.cat([rays, img_idx], 1)  # [1024, 21]
    ...
    k_extract = ['rgb_map', 'acc_map', 'depth', 'depth_var']
...
def render_path(args, render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        rgb, acc, depth, depth_var, extras = render(H, W, focal, chunk=chunk, c2w=c2w[:3, :4], img_idx=img_ids[i],
                                                    **render_kwargs)
        ...
        if savedir is not None:
            ...
            render_depth = to8b(depths[-1] / np.max(depths[-1]))  # save GT img here
            filename = os.path.join(savedir, '{:03d}_depth.png'.format(i))
            imageio.imwrite(filename, render_depth)
...
def render_test(args, train_dl, val_dl, hwf, start, render_kwargs_test):
    ...
    for batch in train_dl:
        img_val = batch['img'].permute(0, 2, 3, 1)  # (B,120,160,3)
    ...
    with torch.no_grad():
        ...
        rgbs, depths = render_path(args, poses_train.to(device), hwf, args.chunk, render_kwargs_test,
    ...
    if args.render_video_train:
        print('Saving trainset as video', rgbs.shape, depths.shape)
        ...
        imageio.mimwrite(moviebase + 'train_depth.mp4', to8b(depths / np.max(depths)), fps=15, quality=8)
    ...
    # views from validation set
    for batch in val_dl:
        img_val = batch['img'].permute(0, 2, 3, 1)  # (1,240,360,3)
        pose_val = torch.zeros(1, 4, 4)
        pose_val[0, :3, :4] = batch['pose'].reshape(3, 4)[:3, :4]  # (1,3,4))
        pose_val[0, 3, 3] = 1.
    ...
    with torch.no_grad():
        torch.set_default_tensor_type('torch.cuda.FloatTensor')
        rgbs, depths = render_path(args, poses_val.to(device), hwf, args.chunk, render_kwargs_test, gt_imgs=images_val,
                                   savedir=testsavedir,
                                   img_ids=index_val)
        torch.set_default_tensor_type('torch.FloatTensor')

    # # Calculate PSNR, SSIM, LPIPS, and save images
    # psnr_test = 0.0
    # ssim_test = 0.0
    # lpips_test = 0.0
    # len_views = len(val_dl)
    #
    # # Create directories for saving images
    # gt_save_dir = os.path.join(args.basedir, args.expname, 'ground_truth')
    # render_rgb_save_dir = os.path.join(args.basedir, args.expname, 'rendered_rgb')
    # render_depth_save_dir = os.path.join(args.basedir, args.expname, 'rendered_depth')
    #
    # # Ensure directories exist
    # os.makedirs(gt_save_dir, exist_ok=True)
    # os.makedirs(render_rgb_save_dir, exist_ok=True)
    # os.makedirs(render_depth_save_dir, exist_ok=True)
    #
    # for i in tqdm(range(len_views)):
    #     gt_img = images_val[i]
    #     render_img = rgbs[i]
    #     depth_img = depths[i]
    #
    #     # Ensure correct shape and type
    #     gt_img = torch.tensor(gt_img).permute(2, 0, 1).float() / 255.0  # (H, W, C) -> (C, H, W), normalize
    #     render_img = torch.tensor(render_img).permute(2, 0, 1).float() / 255.0  # (H, W, C) -> (C, H, W), normalize
    #
    #     # Save ground truth image
    #     gt_img_path = os.path.join(gt_save_dir, '{:05d}_gt.png'.format(i))
    #     torchvision.utils.save_image(gt_img, gt_img_path)  # Normalize to [0, 1] range for saving
    #
    #     # Save rendered RGB image
    #     render_img_path = os.path.join(render_rgb_save_dir, '{:05d}_rendered.png'.format(i))
    #     torchvision.utils.save_image(render_img, render_img_path)  # Normalize to [0, 1] range
    #
    #     # Save rendered depth image with colormap
    #     depth_img_path = os.path.join(render_depth_save_dir, '{:05d}_depth_colored.png'.format(i))
    #     depth_normalized = depth_img / np.max(depth_img)  # Normalize depth to [0, 1] for visualization
    #     colormap = cm.get_cmap('jet')  # Apply 'jet' colormap
    #     depth_colored = colormap(depth_normalized)  # Map depth to colors
    #     depth_colored_rgb = (depth_colored[:, :, :3] * 255).astype(np.uint8)  # Convert to RGB uint8
    #     depth_img_pil = Image.fromarray(depth_colored_rgb)  # Convert to PIL Image
    #     depth_img_pil.save(depth_img_path)
    #
    #     # Calculate metrics
    #     psnr_test += psnr(gt_img, render_img)
    #     ssim_test += ssim(gt_img, render_img)
    #     lpips_test += lpips(gt_img.permute(1, 2, 0), render_img.permute(1, 2, 0), net='alex', format='HWC')
    #
    # # Normalize metrics by the number of views
    # psnr_test /= len_views
    # ssim_test /= len_views
    # lpips_test /= len_views
    #
    # # Print results
    # print("\nPSNR: {:.4f}, SSIM: {:.4f}, LPIPS: {:.4f}".format(psnr_test, ssim_test, lpips_test))

    print('Saved test set')
    if args.render_video_test:
        print('Saving testset as video', rgbs.shape, depths.shape)
        ...
        imageio.mimwrite(moviebase + 'test_depth.mp4', to8b(depths / np.max(depths)), fps=15, quality=8)
