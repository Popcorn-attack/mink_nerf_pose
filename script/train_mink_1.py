...
def train_posenet(args, train_dl, val_dl, model, max_epochs, optimizer, loss_fn, scheduler, device, early_stopping):
    ...
    for epoch in range(max_epochs):
        # Train for one epoch
        avg_train_loss = train_on_epoch(model, val_dl, optimizer, loss_fn, device, epoch, writer, args)
...
def main():
    ...
    model.final_net = nn.Sequential(
        nn.Linear(768, 256),  # First reduce to 256 dimensions
        nn.ReLU(),
        nn.Dropout(0.3),
        nn.Linear(256, 128),
        nn.ReLU(),
        nn.Dropout(0.3),
        nn.Linear(128, 12)  # Output 12 dimensions for pose
    )
